...
n1 = 7;
npts = 10; % num of sources per particle, assume same spherical harmonics discretization
ns = npts*n1^3;
nmpole = n1^3;

nt = 19;
ntm = 10; % mterms, same mpole order for all mps centers. could change depending on complexity, closeness?
...
for i = 1:n1
  for j = 1:n1
    for k = 1:n1
      for ell = 1:npts
        ijk = ijk + 1;
        source(1,ijk) = h*i + (rand(1)-1/2)*2*h/20;
        source(2,ijk) = h*j + (rand(1)-1/2)*2*h/20;
        source(3,ijk) = h*k + (rand(1)-1/2)*2*h/20;
      end
    end
  end
end
...
for i = 1:nmpole
  cmpole(1,i) = source(1,(i-1)*npts+1) + shift; % maybe should choose average as mpole center
  cmpole(2,i) = source(2,(i-1)*npts+1);
  cmpole(3,i) = source(3,(i-1)*npts+1);
end
...
% local = lfmm3d_mps0(nd, eps, nmpole, cmpole, rmpole, mterms, mpole, impole, local, ier);
% post process
...
disp(['mps vs direct error = ', num2str(max(abs(pot-pot2))), ' ']);
...
function local = lfmm3d_mps0(nd, eps, nmpole, cmpole, rmpole, mterms, mpole, impole, local, ier)
...
%c     plot tree structure
       if 0
         opts.ndiv = ndiv;
         srcinfo.sources = cmpole;
...
%c         rescale the multipole expansion
           ...
           tmp(:,:,:,ithd) = mpscale_mex(nd,nterms(ilev+1),cmlexpi,rscpow,tmp(:,:,:,ithd));
%cc        process up down for current box
           mexpupf = zeros(nd,nexptot); mexpdownf = zeros(nd,nexptot);
           ...
           if(npts>0) 
             ...
             for i=1:8
               if(iboxfl(1,i,ithd)>0) 
                 ...
                 for j = jstart:jend
                   k = iboxisort(j,ithd);
                   localsortj = 0*localsort(impolesort(k):(impolesort(k)-1+nd*(mtermssort(k)+1)*(2*mtermssort(k)+1)));
                   localsortj = l3dlocloc_mex(nd,scales(ilev+1),...
                         ...
                         dc,lca);
                   localsort(impolesort(k):(impolesort(k)-1+nd*(mtermssort(k)+1)*(2*mtermssort(k)+1))) = localsort(impolesort(k):(impolesort(k)-1+nd*(mtermssort(k)+1)*(2*mtermssort(k)+1))) + localsortj(:);
                 end
               end
             end
           end
