      ...
      subroutine ptstreemem(src,ns,targ,nt,idivflag,ndiv,nlmin,nlmax,
...
c
        if(nbtot.gt.nbmax) then
          ...
          call treecopy(nbctr,centers,ilevel,iparent,nchild,
     ...
     1        isrcse,itargse)
          ...
          call treecopy(nbctr,centers2,ilevel2,iparent2,
     ...
     1       isrcse2,itargse2)
        ...
        if(irefine.eq.1) then
          ...
          call treerefineboxes(irefinebox,nbmax,
     ...
     2       ilevel,iparent,nchild,ichild)
          ...
          do i=1,nbloc
            ...
            if(irefinebox(i).eq.1) then
              call sortptstochildren(ibox,nbmax,centers,ichild,
     ...
     1       isrcse2,itargse2)
        ...
        call computecoll2(nlevels,nboxes,laddr,boxsize,centers,
     1        iparent,nchild,ichild,iper,nnbors,nbors)

        if(nlevels.ge.2.and.ifunif.ne.1) then
          call ptstreefixlr(centers,nlevels,nboxes,boxsize,nbmax,
...
c
      ...
      subroutine ptstreebuild(src,ns,targ,nt,idivflag,ndiv,
...
c       
        ...
        if(irefine.eq.1) then
          ...
          call treerefineboxes(irefinebox,nboxes,
     1       ifirstbox,nbloc,centers,boxsize(ilev+1),nbctr,ilev+1,
...
c
          do i=1,nbloc
            ibox = ifirstbox+i-1
            if(irefinebox(i).eq.1) then
              call sortptstochildren(ibox,nboxes,centers,
     ...
     1          itree(iptr(5)),targ,nt,itarg,itargse)
      ...
      call computecoll2(nlevels,nboxes0,itree(iptr(1)),boxsize,centers,
     1        itree(iptr(3)),itree(iptr(4)),itree(iptr(5)),iper,
     2        itree(iptr(6)),itree(iptr(7)))

      if(nlevels.ge.2.and.ifunif.ne.1) then
         call ptstreefixlr(centers,nlevels,
...
c
c
      subroutine ptstreesort(n,xys,itree,ltree,nboxes,nlevels,
     ...
     1          itree(iptr(5)),xys,n,ixy,ixyse)
      ...
      subroutine treerefineboxes(irefinebox,nboxes,
...
c
c-------------------------------------------------------------      
      subroutine ptstreefixlr(centers,nlevels,nboxes,
...
c        in the standard format
         ...
         call treerefineboxesflag(iflag,nbmax,laddr(1,ilev),
...
c     Reorganize the tree to get it back in the standard format
      ...
      call ptstreereorg(nboxes,centers,nlevels,laddr,
...
c     Compute colleague information again      
      ...
      do i=1,nboxes
         ...
         enddo
      enddo   
      call computecoll2(nlevels,nboxes,laddr, boxsize,
...
c      in the laddr set and the laddrtail set as well
     ...
     2    nchild,ichild)
         ...
         call treerefineboxesflag(iflag,nbmax,laddrtail(1,ilev),
     1    nbloc,centers,boxsize(ilev+1),nboxes,ilev,ilevel,iparent,
     2    nchild,ichild)
...
c     Compute colleague information again      
     ...
     2                   ichild,iper,nnbors,nbors)
      ...
      subroutine computecoll2(nlevels,nboxes,laddr,boxsize,
...
c           End of computing colleagues of box i
      ...
      subroutine sortptstochildren(ibox,nboxes,centers,
...
c
      ...
      return
       ...
       subroutine treecopy(nb,centers,ilevel,iparent,nchild,ichild,
     1              centers2,ilevel2,iparent2,nchild2,ichild2)
      ...
      subroutine treerefineboxesflag(iflag,nboxes,
     1  ifirstbox,nbloc,centers,bs,nbctr,nlctr,
     2  ilevel,iparent,nchild,ichild)
      implicit none
      integer nboxes,nbloc,nbctr,nlctr
      ...
      subroutine ptstreereorg(nboxes,centers,nlevels,laddr,
...
c     Temporary variables
      ...
      enddo
      call treecopy(nboxes,centers,ilevel,iparent,nchild,
