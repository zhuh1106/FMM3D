...
if nargout>1                  % targ deriv wanted
  ...
  if nargout>2
    Anx = bsxfun(@times, d1./(sqrt(rr).*rr), s.w*(-1/4/pi)); 
    Any = bsxfun(@times, d2./(sqrt(rr).*rr), s.w*(-1/4/pi)); 
    Anz = bsxfun(@times, d3./(sqrt(rr).*rr), s.w*(-1/4/pi)); 
  end
end
...
t.x = r; t.nx = nr; t.w = w; t.N = N;
xin = [0.9; -0.2; 0.1]; xout = [1.9; 0.7; 1.0];    % both "far" from surf
s.x = [xin,xout]; s.w = 1;                         % source pointset (dummy wei)
