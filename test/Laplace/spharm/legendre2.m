function y = legendre2(n,x,normalize)
...
if ~isscalar(n) || ~isreal(n) || ~isfinite(n) || n < 0 || n ~= round(n)
    error(message('MATLAB:legendre:InvalidN'));
end

if ~isreal(x) | ischar(x)| max(abs(x(:))) > 1
    error(message('MATLAB:legendre:InvalidX'))
end
...
elseif strcmp(normalize,'norm')
    ...
    error(message('MATLAB:legendre:InvalidNormalize', normalize));
end
